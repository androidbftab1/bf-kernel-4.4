...
if [ "$CONFIG_ANDROID" == "y" ]; then
	PART1=`mount | grep "/mnt/media_rw/" | sed 's/^.*mnt\/media_rw/\/mnt\/media_rw/g' | sed 's/ type.*$//g' | sed -n '1p'`
else
...
setSmb()
{
	...
	if [ "$CONFIG_ANDROID" == "y" ]; then
		echo -e "$admPW\n$admPW\n" > /data/router/etc/smb_pw_tmp
		echo "smbpasswd -a "$admID""
		smbpasswd -a "$admID" -s < /data/router/etc/smb_pw_tmp
		rm -f /data/router/etc/smb_pw_tmp
	else
		echo -e "$admPW\n$admPW\n" > /etc/smb_pw_tmp
		echo "smbpasswd -a "$admID""
		smbpasswd -a "$admID" -s < /etc/smb_pw_tmp
		rm -f /etc/smb_pw_tmp
	fi	
