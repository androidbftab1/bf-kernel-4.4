...
if [ "$CONFIG_ANDROID" == "y" ]; then
	echo "127.0.0.1 localhost.localdomain localhost" > /data/router/etc/hosts
	echo "$ip $host.ralinktech.com $host" >> /data/router/etc/hosts
else
...
if [ "$dnsp" = "1" ]; then
	if [ "$CONFIG_ANDROID" == "y" ]; then
		dnsmasq -H /data/router/etc/hosts -r /data/router/etc/resolv.conf &
	else
		dnsmasq&
	fi
fi
