...
ifconfig br0 down
brctl delbr br0
apcli_en=`nvram_get 2860 apClient`
if [ "$CONFIG_RT2860V2_AP_APCLI$CONFIG_RT3090_AP_APCLI$CONFIG_RT5392_AP_APCLI$CONFIG_RT5592_AP_APCLI$CONFIG_RT3593_AP_APCLI$CONFIG_MT7610_AP_APCLI$CONFIG_RT3572_AP_APCLI$CONFIG_RT5572_AP_APCLI$CONFIG_RT3680_iNIC_AP_APCLI$CONFIG_RTPCI_AP_APCLI$CONFIG_APCLI_SUPPORT" != "" -a "$apcli_en" = "1" ]; then
...
if [ "$CONFIG_RAETH_QDMA" = "y" -a "$CONFIG_IMQ" = "" ]; then
	echo '##### QDMA LAN/WAN seperate rule remove#####'
	qdma sch 0 0
	qdma m2q 1 0
	qdma sch 8 0
	qdma m2q 2 0
...
# RTDEV_MII support: start mii iNIC after network interface is working
	...
	wan.sh
	lan.sh
	eeeEnb=`nvram_get 2860 eeeEnabled`
	if [ "$eeeEnb" == "1" ]; then
		config-eee.sh 0 1
	else
		config-eee.sh 0 0
	...
	if [ "$CONFIG_RAETH_QDMA" = "y" -a "$CONFIG_IMQ" = "" ]; then
		echo '##### QDMA LAN/WAN seperate rule#####'
		iptables -t mangle -N qdma_prerouting_rule_chain
		iptables -t mangle -A qdma_prerouting_rule_chain ! -i eth3 -j RETURN
		iptables -t mangle -A PREROUTING -j qdma_prerouting_rule_chain
		
		iptables -t mangle -N qdma_postrouting_rule_chain
		iptables -t mangle -A qdma_postrouting_rule_chain ! -o eth3 -j RETURN
		iptables -t mangle -A POSTROUTING -j qdma_postrouting_rule_chain
		
		iptables -t mangle -A qdma_postrouting_rule_chain -j MARK --set-mark 2
		iptables -t mangle -A qdma_postrouting_rule_chain -j RETURN
		iptables -t mangle -A qdma_prerouting_rule_chain -j MARK --set-mark 1
		iptables -t mangle -A qdma_prerouting_rule_chain -j RETURN
		qdma sch 0 0
		qdma m2q 1 0
		qdma sch 8 1
		qdma m2q 2 8
...
#		rplayer &
#	fi	
#	if [ "$CONFIG_USER_BLUEANGEL_BTAGENT" == "y" ]; then
#		sleep 1
#		echo 'btagent ..start'
#		btagent &
#	fi
#	if [ "$CONFIG_USER_BLUEANGEL_BTSNK" == "y" ]; then
#		sleep 1
#		echo 'btsnk ..start'
#		btsnk &
#	fi
#	if [ "$CONFIG_USER_ROUTER_PLAYER_2_0" == "y" ]; then
#		sleep 1
#		echo 'rplayerd ..start'
#		rplayerd &
#	fi
