...
static inline unsigned int GET_TXD_OFFSET(struct END_DEVICE *ei_local,
...
/* struct END_DEVICE *ei_local = netdev_priv(dev_raether); */
	...
	ctx_offset =
	    ((u32)*cpu_ptr - (u32)ei_local->phy_txd_pool) / QTXD_LEN;
...
bool qdma_tx_desc_alloc(void)
{
	...
	pr_err("txd_pool=%p phy_txd_pool=%08X\n", ei_local->txd_pool,
	       ei_local->phy_txd_pool);
...
bool sfq_init(struct net_device *dev)
{
	...
	pr_err("*****sfq_phy0 is 0x%x!!!*******\n", sfq_phy0);
	pr_err("*****sfq_phy1 is 0x%x!!!*******\n", sfq_phy1);
	pr_err("*****sfq_phy2 is 0x%x!!!*******\n", sfq_phy2);
	pr_err("*****sfq_phy3 is 0x%x!!!*******\n", sfq_phy3);
	pr_err("*****sfq_virt0 is 0x%p!!!*******\n", sfq0);
	pr_err("*****sfq_virt1 is 0x%p!!!*******\n", sfq1);
	pr_err("*****sfq_virt2 is 0x%p!!!*******\n", sfq2);
	pr_err("*****sfq_virt3 is 0x%p!!!*******\n", sfq3);
	if (ei_local->chip_name == MT7622_FE) {
		pr_err("*****sfq_phy4 is 0x%x!!!*******\n", sfq_phy4);
		pr_err("*****sfq_phy5 is 0x%x!!!*******\n", sfq_phy5);
		pr_err("*****sfq_phy6 is 0x%x!!!*******\n", sfq_phy6);
		pr_err("*****sfq_phy7 is 0x%x!!!*******\n", sfq_phy7);
...
bool fq_qdma_init(struct net_device *dev)
{
	...
	pr_err("phy_free_head is 0x%x!!!\n", phy_free_head);
	pr_err("phy_free_tail_phy is 0x%x!!!\n", phy_free_tail);
...
int fe_qdma_rx_dma_init(struct net_device *dev)
{
	...
	sys_reg_write(QRX_BASE_PTR_0, phys_to_bus((u32)ei_local->phy_qrx_ring));
...
int ei_qdma_xmit_housekeeping(struct net_device *netdev, int budget)
{
	...
	dma_ptr = (struct QDMA_txdesc *)sys_reg_read(QTX_DRX_PTR);
